...
/**
 * 관리자 대시보드 차트용 집계 매퍼. users/reviews/reports 의 카운트만 읽는 self-contained 매퍼라
 * 도메인 매퍼(공유 코드)를 건드리지 않는다. 전부 GROUP BY 카운트 — 마이그레이션 불필요.
 */
@Mapper
public interface AdminStatsMapper {

    /** since 이후 일자별 가입 수(빈 날짜는 결과에 없음 → 서비스에서 0 채움). */
    List<DailyCount> signupsByDay(@Param("since") LocalDateTime since);
